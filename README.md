...
graph TD
    ...
    B -->|NLP Transcript Text| D[NLP Scam Classifier]
...
- **Multi-Agent Risk Fusion Engine**: Integrates results from NLP transcript classifiers, graph connection weights, speech deepfake spectral audits, and HSV counterfeit note vision checks into a dynamic, weighted threat index.
...
- **FastAPI Gateway Application**: Ingestion routers, orchestrator, and core APIs / OpenAPI documentation (`port 8000`).
...
Run the 16 automated tests locally against active container databases:
```bash
cd backend
...
```bash
backend\.venv\Scripts\python scripts\e2e_demo_flow.py
```
Output:
```text
...
[PASS] Verify Risk Check Explanation (Explanation: High Risk: Phishing / Financial scam signature. ...)
[PASS] Auto Case Linking (Case ID: 1, Title: Campaign Alert: Phone 9988776655, Severity: Medium)
[PASS] PostGIS Heatmap Aggregation (Found 2 grid points)
[PASS] Neo4j Campaign Clustering (Entity '9988776655' mapped to Cluster ID: 0)
[PASS] Bank Risk Assessment (TX ID: 12345, Risk Score: 74.0%, Explanation: ...)
